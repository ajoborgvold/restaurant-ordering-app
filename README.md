...
### Navigation
Every interactive element is accessible with touch, mouse and keyboard navigation. Interactive icons, i.e. the `+` and `-` signs for adding/removing cart items have an `aria-label` conveying there function to assistive technologies.
### HTML structure
...
1. Making all `input fields` required, using regular expressions to define the allowed pattern of the various `input values`, and conveying clear, concise, and accessible information to user about how to make the values valid
...
3. Ensuring that the user can easily close the modal without having to fill out the form and click "pay". Ways to close the modal are: by clicking the `close button`; by clicking anywhere outside the modal; by pressing the `esc key`

### New technologies
This is the first ever project I have built using Tailwind CSS and TypeScript. Along the way, I have learned how to work with a whole lot of new things.
- Tailwind CSS
  1. Installing and configuring Tailwind
  2. Editing/updating `tailwind.config.js`
  3. Various modifiers for e.g.: responsive breakpoints, `hover`/`focus`, `odd`/`even`, `group`, `dark`
- TypeScript
  1. Primitive types, arrays, objects, JSX elements, various event types
  2. Interfaces
